...
func TestParseAndValidate_UnmarshalErrors(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.description, func(t *testing.T) {
			r := repoconfig.Reader{}
			_, err := r.ParseAndValidate([]byte(c.input))
...
func TestParseAndValidate_Successes(t *testing.T) {
...
`,
	...
	for _, c := range cases {
		t.Run(c.description, func(t *testing.T) {
			if c.description == "we parse extra_args for the steps" {
				r := repoconfig.Reader{}
				act, err := r.ParseAndValidate([]byte(c.input))
				Ok(t, err)
